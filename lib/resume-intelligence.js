...
// This runs on flattened PDFs, so keep it limited to phrases that are likely
// real headings. Matching every occurrence of "research" or "project" splits
// titles such as "Independent Research" and "Pneumonia Methylation Project".
const SECTION_MARKER = /(?:education|academic background|work experience|professional experience|employment|research\s+experience|selected projects?|leadership|activities|community(?: service)?|volunteer(?:ing)?|awards(?: & honors?)?|honors?|skills?|technical skills|certifications?|publications?|portfolio|entrepreneurship|ventures?|nonprofit(?: experience)?)/i;
const ROLE_MARKER = /(?:research\s+(?:intern|assistant|fellow)|software\s+engineer|data\s+scientist|project\s+manager|president|founder|co-?founder|director|coordinator|analyst|internship)\b/i;

function resumeLines(text) {
  return String(text || "")
    .replace(/\r\n?/g, "\n")
    // Do not split a real school name simply because it contains a section
    // keyword, such as "American Leadership Academy".
    .replace(/\bleadership\s+(?:academy|high school)\b/gi, (match) => match.replace(/\s/g, "__MERITOS_SPACE__"))
    .replace(/\bindependent\s+research\b/gi, (match) => match.replace(/\s/g, "__MERITOS_SPACE__"))
    .replace(/[•●▪◦◉]/g, "\n")
    .replace(new RegExp(`\\s+(?=${SECTION_MARKER.source}\\b)`, "gi"), "\n")
    .replace(new RegExp(`\\s+(?=${ROLE_MARKER.source}\\b)`, "gi"), "\n")
    .replace(/(?<=[.!?])\s+(?=[A-Z][a-z])/g, "\n")
    .replace(/__MERITOS_SPACE__/g, " ")
    .split(/\n+/)
    .map((raw, index) => ({ raw, line: cleanLine(raw), index }))
...
function boundedEvidenceStatements(value, maxLength = 650) {
  const remaining = cleanLine(value);
  if (remaining.length <= maxLength) return [remaining];
  const statements = [];
  let cursor = remaining;
  while (cursor.length > maxLength) {
    const window = cursor.slice(0, maxLength + 1);
    const sentenceBreak = Math.max(window.lastIndexOf(". "), window.lastIndexOf("; "));
    const wordBreak = window.lastIndexOf(" ");
    const cut = sentenceBreak >= Math.floor(maxLength * 0.45)
      ? sentenceBreak + 1
      : Math.max(wordBreak, Math.floor(maxLength * 0.75));
    statements.push(cursor.slice(0, cut).trim());
    cursor = cursor.slice(cut).trim();
  }
  if (cursor) statements.push(cursor);
  return statements;
}
...
export function categoryForHeading(heading) {
  ...
  if (EDUCATION_MARKERS.test(normalized)) return "Education";
...
export function categoryForStatement(statement) {
  ...
  if (EDUCATION_MARKERS.test(value) || /\b(class of|expected graduation|graduating|gpa|coursework|\d{1,2}(?:th|st|nd|rd) grade)\b/i.test(value)) return "Education";
...
export function extractResumeProfile(text) {
  const originalLines = resumeLines(text);
...
export function extractResumeEvidence(text) {
  ...
  const flushSection = () => {
    ...
    for (const line of sectionLines) {
      // Dates and hyphens are normal inside achievement bullets. Only split
      // when a new role or a new named research project begins; otherwise the
      // title, methods, and result remain one supported fact.
      const roleHeader = /\b(research\s+(?:intern|assistant|fellow)|(?:software|data|biomedical|research)\s+(?:engineer|scientist|analyst)|(?:co-?)?(?:founder|president|director|manager|coordinator)|(?:teaching|laboratory)\s+assistant)\b/i.test(line);
      const researchProjectHeader = activeSection === "Research experience"
        && current.length >= 2
        && line.length <= 120
        && !/[.!?]$/.test(line)
        && /\b(independent research|project|modeling|study|analysis)\b/i.test(line);
      if ((roleHeader || researchProjectHeader) && current.length >= 2) {
        chunks.push(current);
        current = [];
      }
      current.push(line);
    ...
    for (const chunk of chunks) {
      for (const statement of boundedEvidenceStatements(chunk.join(" "))) {
        const key = statement.toLowerCase();
        if (statement.length < 10 || seen.has(key)) continue;
        seen.add(key);
        const inferredCategory = categoryForStatement(statement);
        // Recognition wording is stronger evidence than generic terms such as
        // "community" inside the award title (e.g. Community Changemaker Award).
        const category = /\b(award|honou?r(?:able)?|scholarship|distinction|dean'?s list|finalist|winner|medal|champion|gold|silver|bronze)\b/i.test(statement)
          ? "Award or distinction"
          : narrativeCategories.has(inferredCategory)
          && (activeSection === "Education" || activeSection === "Other resume evidence")
            ? inferredCategory
            : activeSection;
        candidates.push({ statement, category });
      }
    }
    sectionLines = [];
  };
  ...
  for (const { line } of resumeLines(text)) {
    if (!line || line.length > 1_600 || !/[a-z]/i.test(line)) continue;
    const section = categoryForHeading(line);
    const isResearchProjectTitle = activeSection === "Research experience"
      && section === "Project or impact"
      && /\bproject\b/i.test(line)
      && !SECTION_HEADING.test(line);
    const looksLikeHeading = line.length <= 48 && (
      SECTION_HEADING.test(line)
      || (line.length >= 3 && line === line.toUpperCase())
      || /:$/.test(line)
      // PDF text extractors often preserve a heading's title case but lose its
      // typography (e.g. "Technical Skills" or "Honors & Awards").
      || Boolean(section && !isResearchProjectTitle && /^[A-Z][A-Za-z &/]+$/.test(line))
    );
    if (looksLikeHeading) {
      flushSection();
      if (section) activeSection = section;
      // Schools are often styled in all caps and therefore look like a section
      // heading after PDF extraction. Preserve the institution as evidence too.
      if (section === "Education" && EDUCATION_MARKERS.test(line)) {
        const key = line.toLowerCase();
        if (!seen.has(key)) {
          seen.add(key);
          candidates.push({ statement: line, category: "Education" });
        }
      }
      continue;
    }
    // A surprising number of résumé PDFs omit the visual section break when
    // extracting text. Do not let a school line swallow the next research or
    // leadership role just because the previous detected heading was Education.
    // The same rule preserves a nonprofit role and its following impact line.
    const inlineCategory = categoryForStatement(line);
    const isNarrativeTransition = narrativeCategories.has(inlineCategory)
      && inlineCategory !== "Skills or certification"
      && inlineCategory !== activeSection
      && (
        activeSection === "Education"
        || activeSection === "Other resume evidence"
        || /\b(research\s+(?:intern|assistant|fellow)|(?:co-?)?(?:founder|president|director|manager|coordinator)|(?:software|data|biomedical|research)\s+(?:engineer|scientist|analyst)|(?:teaching|laboratory)\s+assistant)\b/i.test(line)
      );
    if (activeSection && isNarrativeTransition) {
      flushSection();
      activeSection = inlineCategory;
    }
    if (activeSection) {
