...
export function extractResumeEvidence(text) {
  ...
  const flushSection = () => {
    ...
    for (const line of sectionLines) {
      ...
      if (roleHeader && current.length >= 2) {
        chunks.push(current);
        current = [];
      }
      current.push(line);
  ...
  for (const { line } of resumeLines(text)) {
    ...
    const isNarrativeTransition = narrativeCategories.has(inlineCategory)
      ...
      && /\b(research|laboratory|lab\b|genomics|bioinformatics|methylation|president|founder|director|captain|chair|led|founded|coordinated|organized|managed|volunteer|community|outreach|project|built|developed|designed|created|implemented|intern|assistant|engineer|analyst)\b/i.test(line);
