...
const NARRATIVE_PATTERN = /research|leadership|initiative|project|impact|community|volunteer|service|award|honou?r|achievement|distinction|education|coursework|work experience|employment|career|startup|venture|entrepreneur|founder|nonprofit|teaching|creative|portfolio|design|skills?|experience|extracurricular|outreach|fundrais|what .*bring|ideas?.*(?:grow|promot)/i;
...
const EVIDENCE_HINTS = {
  ...
  entrepreneurship: /startup|venture|entrepreneur|founder|co-?founder|business|company|launched/i,
  nonprofit: /nonprofit|non-profit|charity|philanthrop|mission-driven|social impact/i,
  work: /professional experience|employment|work(?:ed|ing)?|job|career|associate|specialist|consultant|manager|engineer|analyst/i,
  teaching: /teacher|teaching|instructor|educator|lesson|classroom|student learning/i,
  creative: /creative|artist|artistic|designer|portfolio|film|illustrat|writing sample/i,
  skills: /skills?|proficien|certifi|technolog|programming|technical/i,
  experience: /experience|role|intern|employment|research|project|leadership|community/i,
  extracurriculars: /extracurricular|activities|club|organization|association/i,
  contribution: /what .*bring|contribut|value|strength|impact/i,
  growth_ideas: /ideas?|grow|growth|promot|execute|fundrais|strategy/i,
  outreach_strategy: /outreach|recruit|signups?|social media|promot/i,
  writing_experience: /writing|essay|prompt|publication|content/i,
};
...
export function selectRelevantEvidence(field, evidence) {
  const question = `${field.label} ${field.description ?? ""} ${field.name ?? ""}`.toLowerCase();
  ...
  const compatibleIntents = {
    ...
    skills: ["research", "project", "work"],
    contribution: ["research", "leadership", "project", "skills"],
  };
  return dedupeEvidence(evidence
    .map((item) => {
      const value = `${item.category} ${item.statement}`;
      const score = requested.reduce((total, intent) => {
        const direct = EVIDENCE_HINTS[intent].test(value);
        const compatible = (compatibleIntents[intent] || []).some((related) => EVIDENCE_HINTS[related].test(value));
        return total + (direct ? 1 : compatible ? 0.65 : 0);
      }, 0);
      return { item, score };
    })
...
export function buildDraftingPrompt({ field, page = {}, evidence }) {
  ...
  return {
    developer: [
      ...
      "Page opportunity context and Human Profile guidance help choose relevant verified evidence, but are not evidence themselves and must never introduce a factual claim or a reason for applying.",
    ...
    user: JSON.stringify({
      ...
      page: {
        ...
        opportunityContext: cleanDraftingText(page?.opportunityContext, 2_000),
      },
