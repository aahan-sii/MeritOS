...
async function scanExternalBoards(query: string) {
  ...
  const candidates = [
    ...
    ...((jobicy?.jobs || []).map((job: Record<string, unknown>) => ({ company: String(job.companyName || "Employer"), title: String(job.jobTitle || "Open role"), location: String(job.jobGeo || "Remote"), url: String(job.url || ""), source: "Jobicy", repository: "https://jobicy.com/" }))),
    ...((muse?.results || []).map((job: Record<string, unknown>) => { const company = job.company && typeof job.company === "object" ? String((job.company as Record<string, unknown>).name || "Employer") : "Employer"; const locations = Array.isArray(job.locations) ? job.locations.map((item) => typeof item === "object" && item ? String((item as Record<string, unknown>).name || "") : "").filter(Boolean).join(", ") : "Check listing"; const refs = job.refs && typeof job.refs === "object" ? job.refs as Record<string, unknown> : {}; return { company, title: String(job.name || "Open role"), location: locations, url: String(refs.landing_page || ""), source: "The Muse", repository: "https://www.themuse.com/search/" }; })),
    ...((hackerNews?.hits || []).map((hit: Record<string, unknown>) => { const text = String(hit.comment_text || "").replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim(); return { company: String(hit.author || "Hacker News"), title: text.slice(0, 120) || String(hit.story_title || "Who is hiring opportunity"), location: /remote/i.test(text) ? "Remote mentioned" : "Check post", url: `https://news.ycombinator.com/item?id=${encodeURIComponent(String(hit.objectID || ""))}`, source: "Hacker News", repository: "https://hn.algolia.com/api" }; })),
  ];
