...
import { createStory } from "@/lib/profile-intelligence";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const generated = await createStory({ target, lens, claims: verified });
    const now = new Date();
    ...
    await recordAuditEvent({
      ...
      entityId: storyId,
      action: "generated",
      detail: { lens, sourceClaimIds: generated.sourceClaimIds },
    });
    return NextResponse.json({
